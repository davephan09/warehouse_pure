<thead>
    ...
    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0">
        <th class="min-w-125px" data-priority="1">{{trans('unit.unit_name')}}</th>
...
<tbody class="fw-bold text-gray-600">
    ...
    <tr>
        ...
        <td>{{date('H:i, d M Y', strtotime($unit->created_at))}}</td>
        <td class="text-end">
            @can('unit.update')
            <button class="btn btn-icon btn-active-light-primary w-30px h-30px me-3 update-btn" data-bs-toggle="modal" data-id="{{$unit->id}}"
                data-bs-target="#kt_modal_update_unit" title="{{__('common.update')}}">
            ...
            @endcan
            @can('unit.delete')
            <button class="btn btn-icon btn-active-light-primary w-30px h-30px delete-btn" data-id="{{$unit->id}}" title="{{__('common.delete')}}"
